...
def write_sfs(unmasked_sfs, prefix):
    outdir = f"../dadi/{prefix}"
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    num_bs = len(unmasked_sfs)
    index = 1
    while index <=num_bs:
        for sfs in unmasked_sfs:
            name = prefix + "_" + str(index)
            print(name)
            sfs.to_file(f"{outdir}/{name}.sfs")
            index+=1
...
def main():
    ...
    # write bootstrapped SFS to files for further analysis
    write_sfs(elata_4f_unmasked, "elata_4f")
    write_sfs(elata_0f_unmasked, "elata_0f")
    write_sfs(biennis_4f_unmasked, "biennis_4f")
    write_sfs(biennis_0f_unmasked, "biennis_0f")
    ...
    # plot the spectra
    plot_sfs([elata_4f_df,elata_0f_df], ['b'], "../figures/elata_coding_bootstraps_sfs")
    plot_sfs([biennis_4f_df,biennis_0f_df], ['r'], "../figures/bienns_coding_bootstraps_sfs")
    plot_sfs([biennis_4f_df,biennis_0f_df,elata_4f_df, elata_0f_df],['r', 'b'],"../figures/codingsites_bootstraps_sfs")
