...
dd4f = dadi.Misc.make_data_dict_vcf("../vcfs/variants.32samples.0.5missing.recombined.4folddegen.recode.vcf.gz", "../../popmap_nogrand.txt")
dd0f = dadi.Misc.make_data_dict_vcf("../vcfs/variants.32samples.0.5missing.recombined.0folddegen.recode.vcf.gz", "../../popmap_nogrand.txt")

#fs_biennis = dadi.Spectrum.from_data_dict(dd, ['biennis'], projections = [18], polarized = False)
fs4f_biennis = dadi.Spectrum.from_data_dict(dd4f, ['biennis'], projections = [14], polarized = False, mask_corners=False)
fs0f_biennis = dadi.Spectrum.from_data_dict(dd0f, ['biennis'], projections = [14], polarized = False, mask_corners=False)
#fs_elata = dadi.Spectrum.from_data_dict(dd, ['elata'], projections = [20], polarized = False)
fs4f_elata = dadi.Spectrum.from_data_dict(dd4f, ['elata'], projections = [20], mask_corners=False, polarized = False)
fs0f_elata = dadi.Spectrum.from_data_dict(dd0f, ['elata'], projections = [20], polarized = False, mask_corners=False)
...
def unmask_sfs(sfs, filename):
    # get sfs with zero allele count category unmasked and remove freq categories > 0.5
    sfs.mask[0] = False
    sfs = dadi.Spectrum(sfs.data, data_folded=sfs.folded, mask=sfs.mask, fill_value=0, dtype=int)
    sfs.to_file(filename)
...
elata_snm_unmasked = unmask_sfs(elata_snm, "../dadi/elata_snm.sfs")
elata_4f_unmasked = unmask_sfs(fs4f_elata, "../dadi/elata_fs4f.sfs")
elata_0f_unmasked = unmask_sfs(fs0f_elata, "../dadi/elata_fs0f.sfs")
biennis_snm_unmasked = unmask_sfs(biennis_snm, "../dadi/biennis_snm.sfs")
biennis_4f_unmasked = unmask_sfs(fs4f_biennis, "../dadi/biennis_fs4f.sfs")
biennis_0f_unmasked = unmask_sfs(fs0f_biennis, "../dadi/biennis_fs0f.sfs")
